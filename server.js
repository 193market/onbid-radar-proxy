...
async function proxyGetNew(res, endpoint, normalizer, params) {
  ...
  // sido 필터 시 충분한 데이터를 가져와 서버에서 필터링 (캠코 API lctnSdnm 파라미터 미지원)
  const fetchRows = sido ? 200 : Math.max(1, Math.ceil(numOfRows / PRPT_DIV_CDS.length));
  const fetchPage = sido ? 1 : pageNo;
  ...
  try {
    ...
    // sido 서버 필터링 (캠코 API가 지역 파라미터 미지원으로 서버에서 직접 필터)
    if (sido) {
      allItems = allItems.filter(i => (i.sidoNm || '').includes(sido) || sido.includes(i.sidoNm || ''));
      totalCount = allItems.length;
      const start = (pageNo - 1) * numOfRows;
      allItems = allItems.slice(start, start + Number(numOfRows));
    }
